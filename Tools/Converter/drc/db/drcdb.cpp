...
void DRCDB::LoadRecentMediations(MediatorArg arg)
{
    ...
    QSqlQuery Mediation_query(database);
    QString Mediation_command_string = "Select * from Mediation_Table order by UpdatedDateTime desc";
...
MediationProcessVector* DRCDB::getMediationProcesses()
{
    QSqlQuery query(database);

    QString command = QString("Select * from Mediation_table");

    if(!this->ExecuteCommand(command, query))
    {
        qDebug()<<"Error in getMediationProcesses";
    }
...
void DRCDB::InsertMediationVector(MediationProcessVector* MediationProcesses)
{
    qDebug() << "Inserting " <<  MediationProcesses->size() << " mediations..... ";
    for (int i = 0; i < MediationProcesses->size(); i++)
    {
        qDebug() << "Inserting Mediation " << i + 1 << " out of " << MediationProcesses->size() << "..... ";
        InsertMediation(MediationProcesses->at(i));
        //qDebug() << "DONE" << endl;
    }
    qDebug() << "Done inserting " <<  MediationProcesses->size() << " mediations!. ";
}

//This function is a copy of the load database function, with a few modifications for the conversion
//Additions:
//CreateMediationTable was changed to CreateMediationTable2
void DRCDB::LoadDatabase2(QString filename)
{
    OpenDatabase(filename);
    ...
    CreatePersonTable(person_table_name);
    CreateMediationTable2(mediation_table_name);
    CreateSessionTable(session_table_name);
    CreateClientTable(client_table_name);
    CreateNotesTable(notes_table_name);
    CreateClientSessionTable(client_session_table_name);
...
//This function is a copy of the CreateMediationTable function, with a few modifications for the conversion
//Additions:
//Added colums:
bool DRCDB::CreateMediationTable2(const QString& mediation_table_name)
{
    QVector<QString> mediation_table_columns;
    mediation_table_columns.push_back(QString("Process_id integer primary key autoincrement null"));
    mediation_table_columns.push_back(QString("DisputeType integer"));
